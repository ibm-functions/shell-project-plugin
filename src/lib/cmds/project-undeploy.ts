...
const usage = `Undeploy project.

\tundeploy [<manifest.yml>]

Required parameters:
\t<manifest.yml>       the project configuration file (default "manifest.yml")`;

async function doUndeploy(block, nextBlock, _3, { ui, errors }, _4, _5, _6, argv) {
    if (argv.help) {
    ...
    return execSync(`${wskdeploy} undeploy --managed -m ${file}`).toString();
}
